...
const ParagraphEngine = ({ onGoHome, autoStart = false }) => {
  const { saveGameStats, isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    if (gameState === 'finished' && isLoggedIn) {
      ...
      // Calculate approximate WPM (assuming average word length of 5 characters)
      const wordsTyped = Math.floor(totalCharacters / 5);
  ...
  }, [gameState, isLoggedIn, totalCharacters, totalErrors, startTime, saveGameStats]);
