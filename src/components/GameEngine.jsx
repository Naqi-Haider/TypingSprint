...
// Session ID to prevent double loader render in Strict Mode
let sessionId = 0;

const GameEngine = ({ onGoHome, autoStart = false }) => {
  const { updateUserStats, saveGameStats, user, isLoggedIn } = useAuth();
  ...
  const [mistakeFlash, setMistakeFlash] = useState(false); // For red flash effect
  ...
  // Start game - shows Ready-Set-Go loader (or skips if autoStart since App already showed it)
  const startGame = useCallback((skipLoader = false) => {
    // Prevent double initialization
    if (isInitializing.current) return;
    isInitializing.current = true;

    // Increment session ID to ensure unique loader key
    sessionId += 1;
    ...
    hasSavedStats.current = false; // Reset for new game

    // Skip loader if already shown by parent (autoStart from main page)
    if (skipLoader) {
      setGameState('playing');
      isInitializing.current = false;
    } else {
      // Start with loading state (Ready-Set-Go)
      setGameState('loading');
    }
  }, [generateWords]);

  // Handle loading completion (Ready-Set-Go finished)
  const handleLoadingComplete = useCallback(() => {
    setGameState('playing');
    isInitializing.current = false; // Reset initialization flag
  }, []);

  // Auto-start effect - runs once on mount if autoStart is true
  // Skip the loader since App.jsx already shows it before mounting GameEngine
  useEffect(() => {
    if (autoStart && gameState === 'idle' && !isInitializing.current) {
      startGame(true); // Skip loader - App already showed it
    }
  }, [autoStart]); // Only depend on autoStart, not gameState or startGame
  ...
  useEffect(() => {
    if (timeLeft === 0 && gameState === 'finished' && !hasSavedStats.current) {
      hasSavedStats.current = true; // Mark as saved to prevent double-firing
  ...
  }, [timeLeft, gameState, totalWords, totalCharacters, incorrectCharacters, isLoggedIn, user, updateUserStats, saveGameStats]);
  ...
  // Calculate current score for live display
  const calculateCurrentScore = () => {
    const correctChars = totalCharacters - incorrectCharacters;
    const accuracyRatio = totalCharacters > 0 ? correctChars / totalCharacters : 0;
    return Math.round((correctChars * accuracyRatio) + (totalWords * 10));
  };

  // Calculate WPM, accuracy, and score for results
  const calculateStats = () => {
    ...
    // Calculate score using accuracy-weighted formula
    // accuracy = correctChars / (correctChars + mistakes)
    // score = (correctChars * accuracy) + (correctWords * 10)
    const accuracyRatio = totalCharacters > 0 ? correctChars / totalCharacters : 0;
    const score = Math.round((correctChars * accuracyRatio) + (totalWords * 10));

    // Get and update best score from localStorage
    const storedBestScore = parseInt(localStorage.getItem('speedBulletBestScore') || '0');
    const isNewHighScore = score > storedBestScore;
    const bestScore = Math.max(storedBestScore, score);
    if (isNewHighScore) {
      localStorage.setItem('speedBulletBestScore', score.toString());
    }

    return { wpm, accuracy, score, bestScore, isNewHighScore };
  };
  ...
  return (
    <div className="game-engine">
      <AnimatePresence mode="wait">
        ...
        {gameState === 'loading' && (
          <TerminalLoader key={`loader-${currentSessionId.current}`} onComplete={handleLoadingComplete} />
        )}

        {gameState === 'playing' && (
          <div className="game-screen" key="game">
            {/* Header with Timer and Score in combined box */}
            <div className="game-header">
              <div className="timer-score-box">
                <div className={`timer-section ${getTimerClass()} ${showPenalty ? 'penalty' : ''}`}>
                  <span className="timer-value">{timeLeft}s</span>
                  <span className="timer-label">Time Left</span>
                  {showPenalty && <span className="penalty-indicator">-1</span>}
                </div>
                <div className="score-section">
                  <svg className="score-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill="currentColor" />
                  </svg>
                  <span className="score-value">{calculateCurrentScore()}</span>
            ...
            <div className="game-stats glass">
              ...
              <div className={`stat mistake-stat ${mistakeFlash ? 'flash-error' : ''}`}>
